...
class OscLow extends Component {
	constructor(props) {
	...
	scheduleNote(beatNumber, time){
 		...
 		if(this.props.inWindow.size > beatNumber) //to account for shrinking window size
		{
			...
			var freq = 0.1;
			if(this.props.inWindow.get(beatNumber)  != 0 && this.props.inWindow.get(beatNumber) != null && this.props.inWindow.get(beatNumber))
				freq = parseFloat(this.props.ff) + scaled_data * this.props.scale_factor;
			...
			if(isNaN(freq) || this.props.inWindow.get(beatNumber) < 0.05)
				freq = 0.1;
			this.osc.frequency.exponentialRampToValueAtTime(freq,time);
